...
<fieldset class="fieldset-no-account-sign-in">
...
<?php
	if ( $registration_enabled ) :
		printf( __( 'If you don&rsquo;t have an account you can %screate one below by entering your email address/username. Your account details will be confirmed via email.', 'wp-job-manager' ), $account_required ? '' : __( 'optionally', 'wp-job-manager' ) . ' ' );
	elseif ( $account_required ) :
		echo apply_filters( 'submit_job_form_login_required_message',  __('You must sign in to create a new listing.', 'wp-job-manager' ) );
	endif;
?>
</div><!-- end .field account-sign-in -->
</fieldset>
<?php
	if ( $registration_enabled ) :
