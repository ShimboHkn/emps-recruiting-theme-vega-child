...
<?php
	if($vega_wp_blog_feed_meta_date == 'Y') {
		$date_format = get_option('date_format');
		$temp[] = __('Posted: ', 'vega') . get_the_date($date_format);
	}
	if($vega_wp_blog_feed_meta_category == 'Y') {
		$temp[] = __('Under: ', 'vega'). get_the_category_list(', ');
	}
	if($vega_wp_blog_feed_meta_author == 'Y') {
...
<?php
if($vega_wp_blog_feed_buttons == 'Y') :
?>
<!-- Post Buttons -->
<div class="recent-entry-buttons">
<?php
	$readmore = esc_html(vega_wp_get_option('vega_wp_blog_feed_readmore_text'));
	if($readmore != '') {
?>
<a href="<?php the_permalink(); ?>" class="btn btn-primary-custom btn-readmore"><?php echo $readmore; ?></a>
<?php
	}
